...
def policy_evaluation(env, policy, gamma=0.9, theta=0.01):
    V = np.zeros(env.observation_space.n)  # مقدار اولیه همه حالات صفر است
    while True:
        delta = 0
        for s in range(env.observation_space.n):
            v = V[s]
            # محاسبه احتمالات انتخاب کردن هر کنش در حالت s
            action_probs = [policy[s][a] for a in range(env.action_space.n)]
            # محاسبه ارزش حالت s با استفاده از معادله بلمن
            V[s] = sum(action_probs[a] * sum(p * (r + gamma * V[s_]) for (s_, r, p, _) in env.P[s][a]) for a in range(env.action_space.n))
...
def policy_improvement(env, V, gamma=0.9):
    ...
    for s in range(env.observation_space.n):
        # محاسبه مقدار Q برای هر کنش در حالت s
        q_values = [sum(p * (r + gamma * V[s_]) for (s_, r, p, _) in env.P[s][a]) for a in range(env.action_space.n)]
        # پیدا کردن بهترین کنش(کنش‌های) در حالت s
        best_actions = np.argwhere(q_values == np.max(q_values)).flatten()
        # تقسیم احتمال ۱ به تعداد بهترین کنش‌ها
        n_best_actions = len(best_actions)
        for a in best_actions:
            policy[s][a] = 1 / n_best_actions
    return policy

def policy_iteration(env, gamma=0.9, theta=0.01, max_iter=30):
    # ساخت خط‌مشی تصادفی
...
def first_visit_mc_prediction(env, policy, num_episodes, gamma):
# Initialize empty dictionaries to store returns and values
    ...
    # Initialize episode and get initial state
        episode = []
        state = env.reset()
    ...
    # Generate an episode following the policy
        ...
        # Update the value estimates for each visited state
    ...
    for state in visited_states:
        ...
        # Calculate the return from that point onwards
        G = sum([x[2]*(gamma**i) for i,x in enumerate(episode[first_occurrence_idx:])])
        # Update the returns sum and count for the state
        returns_sum[state] += G
...
def every_visit_mc_prediction(env, policy, num_episodes, gamma):
    ...
    for i in range(num_episodes):
        ...
        for state in visited_states:
            ...
            # Calculate the total return from the first occurrence onwards
            G = sum([x[2]*(gamma**i) for i,x in enumerate(episode[first_occurrence_idx:])])
